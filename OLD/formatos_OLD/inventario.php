<?php
	...
	if ($valor == 'GENERAL'){
		...
		$query = "SELECT NOMBRE_PRODUCTO, CATEGORIA, EMPAQUE, ULTIMO_PROVEEDOR
	...
	elseif ($valor == 'ABARROTES_GRAL'){
		...
		$query = "SELECT NOMBRE_PRODUCTO, CATEGORIA, EMPAQUE, ULTIMO_PROVEEDOR, ULT_FACT_HACE, CANT_FACT, UNID_FACT
	...
	elseif ($valor == 'JARCIERIA'){
		...
		$query = "SELECT NOMBRE_PRODUCTO, CATEGORIA, EMPAQUE, ULTIMO_PROVEEDOR, OBSERVACIONES 
	...
	elseif ($valor == 'PATRONA'){
		$pdf->titulo_pdf = "CHILES Y SEMILLAS SAN JOSE - INVENTARIO PATRONA";	
		$query = "SELECT NOMBRE_PRODUCTO, CATEGORIA, EMPAQUE, ULTIMO_PROVEEDOR
				  FROM QRY_INVENTARIO_PATRONA
				  ORDER BY CATEGORIA ASC, NOMBRE_PRODUCTO ASC";
		$resultado =  $bd -> query($query);					
	}
	...
	$pdf->Cell(30);
	$pdf->Cell(160,6, utf8_decode('Fecha de realización del inventario:'),1,0,'L');
	$pdf->Ln(6);
	
	$pdf->Cell(30);
	$pdf->Cell(80,6, utf8_decode('Realizó:'),1,0,'L');
		
	$pdf->Cell(80,6, utf8_decode('Revisó:'),1,0,'L');
	$pdf->Ln(10);
	...
	$pdf->SetFont('Arial','B',10);
	$pdf->Cell($margen);
	$pdf->Cell(135,$linea,'NOMBRE DEL PRODUCTO',1,0,'C',1);	
	$pdf->Cell(15,$linea,'FTE',1,0,'C',1);
	$pdf->Cell(15,$linea,'BOD',1,0,'C',1);
	$pdf->Cell(15,$linea,'PED',1,0,'C',1);
	$pdf->Ln($linea);
	...
	while($row = $resultado->fetch_assoc()){		
		...
		if ($i >= $filas_max){
			...
			$pdf->Cell(180,$linea,utf8_decode($subtitulo),1,0,'C', 1);
			$pdf->Ln($linea);*/
		...
		if (strlen($cadena_fact) <= 3) {
			$cadena_fact = 'SF';
		}
		
		$pdf->Cell(17, $linea, $cadena_fact, 1, 0, 'R');
		// aqui va lo de facturacion
		*/
		...
		$pdf->setXY(152-$tamano_cadena, $y+3);			
		$pdf->Write(0, $row['EMPAQUE']);		
		
		$pdf->setXY($x, $y);
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(15, $linea,' ',1,0,'C');
		$pdf->Cell(15, $linea,' ',1,0,'C');		
		$pdf->Cell(15, $linea,' ',1,0,'C');
		$pdf->Ln($linea);
