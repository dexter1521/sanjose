...
function addList()
{
	var r = prompt(_mtt.lang.get('addList').toUpperCase(), _mtt.lang.get('addListDefault').toUpperCase()).toUpperCase();
	if(r == null) return;
...
function prepareTaskStr(item, noteExp)
{
	...
	return '<li id="taskrow_'+id+'" class="' + (item.compl?'task-completed ':'') + item.dueClass + (item.note!=''?' task-has-note':'') +
		...
		'<div class="task-through">'+preparePrio(prio,id)+'<span class="task-title">'+prepareHtml(item.title)+"<br>"+'<small>'+item.dateInlineTitle+'</small>'+"<br>"+'<small>'+item.dateCompletedInlineTitle+'</small>'+
...
function submitNewTask(form)
{
	...
	_mtt.db.request('newTask', { list:curList.id, title: form.task.value.toUpperCase(), tag:_mtt.filter.getTags() }, function(json){
