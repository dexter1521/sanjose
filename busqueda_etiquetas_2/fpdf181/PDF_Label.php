	...
	function Make_Labels($formato, $text_1, $text_2, $text_3, $text_4, $font_size, $notas) {
		...
		if ($formato == 'J5260'){
			...
			for($i=1; $i<=30; $i++) {
				...
				//$this->SetFillColor(255, 255, 0); // amarillo puro							
				//$this->SetFillColor(255, 255, 75); // amarillo 65%							
				//$this->Rect($_PosX - 6, $_PosY - 5, 70, 24.5, 'D');
				//$this->Rect($_PosX - 6, $_PosY - 5, 70, 24, 'F');
				
				$this->Image('img/logo_4.png', $_PosX - 0.1 + $offset_x, $_PosY + 3.2 + $offset_y, 17);
				...
				$this->SetFont('Arial', 'B');
				$this->Set_Font_Size(7);
				$this->SetXY($_PosX + 7 + $offset_x, $_PosY + 16.7 + $offset_y);
				$this->MultiCell($this->_Width - $this->_Padding, $this->_Line_Height, $text_3, 0, 'C');					
				
				$this->SetTextColor(0, 0, 0);				
				$this->SetFont('Arial','B');
				$this->Set_Font_Size(7);
				$this->SetXY($_PosX + 7 + $offset_x, $_PosY + 19 + $offset_y);
				$this->MultiCell($this->_Width - $this->_Padding, $this->_Line_Height, $text_4, 0, 'C');
				
				
				// ESQUELETO NUNCA MOVER SOLO PRUEBAS
				//$this->SetLineWidth(0.3);
				//$this->SetDrawColor(0, 0, 0);
				//$this->Rect($_PosX -6, $_PosY - 5, 74, 29, 'F');
				//$this->Rect($_PosX - 6, $_PosY - 6, 72, 25, 'D');
		...
		elseif ($formato == 'J5267'){
			...
			for($i=1; $i<=80; $i++) {
				...
				//$this->SetFillColor(255, 255, 0); // amarillo puro
				//$this->SetFillColor(255, 255, 75); // amarillo 65%
				//$this->Rect($_PosX - 5, $_PosY - 1 , 50, 17, 'F');
				
				$this->Image('img/logo_5.png', $_PosX - 1.5 + $offset_x, $_PosY + 0.5 + $offset_y, 9);
				...
				$this->Set_Font_Size($font_size);
				$this->Set_Font_Size(7);				
				$this->SetXY($_PosX + $offset_x + 3, $_PosY + $offset_y + 0.2);				
				$this->MultiCell($this->_Width - $this->_Padding, $this->_Line_Height, $text_1, 0, 'C');
				...
				$this->Rect($_PosX+$offset_x+7, $_PosY + 3 + $offset_y, 33.5, 6, 'DF');
				...
				// ajustes en las fuetes
				if ($font_size <= 7){
					$this->SetXY($_PosX+$offset_x+7, $_PosY + 3 + $offset_y);
				}
				
				else{
					$this->SetXY($_PosX+$offset_x+7, $_PosY + 4 + $offset_y);
				}
			
				$this->MultiCell($this->_Width - $this->_Padding - 8.5, $this->_Line_Height, $text_2, 0, 'C');
				
				
				$this->SetTextColor(0, 0, 0);
				$this->SetFont('Arial','B');
				$this->Set_Font_Size(6);
				$this->SetXY($_PosX+$offset_x + 3.5, $_PosY + 9 + $offset_y);
				$this->MultiCell($this->_Width - $this->_Padding, $this->_Line_Height, $text_4, 0, 'C');
								
				// ESQUELETO NUNCA MOVER SOLO PRUEBAS
				//$this->SetLineWidth(0.1);
				//$this->SetDrawColor(0, 0, 0);
				//$this->Rect($_PosX - 2.5, $_PosY, 45, 13, 'D');
